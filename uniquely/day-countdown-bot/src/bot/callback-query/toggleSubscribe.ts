...
import {addChat, subscribe, toggleSubscribe} from '../../util/chat.js';
...
bot.defineCallbackQueryHandler('toggleSubscribe', async (context) => {
  ...
  if (isSubscribed == null) {
    ...
    await subscribe(context.chatId);
    isSubscribed = true;
  }
  ...
  if (isSubscribed === true) {
    ...
    notifyAdmin(
        message('admin_notify_subscribed').replace('${chatId}', context.update.callback_query!.message!.chat.id + ''),
    );
  }
  else {
    context.answerCallbackQuery({text: message('unsubscribed_successfully_message')});
    notifyAdmin(
        message('admin_notify_unsubscribed').replace('${chatId}', context.update.callback_query!.message!.chat.id + ''),
    );
  }
